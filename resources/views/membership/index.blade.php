@extends('layout.app')
@section('title', 'Customer Membership List')
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <div class="card">
                            ...
                            <tbody>
                                @foreach ($data as $item)
                                    <tr>
                                        ...
                                        {{-- <td>{{ $item->expiration_period == 0 || $item->expiration_period == null ?
                                            'Lifetime' : $item->expiration_period . ' Month(s)' }}
                                        </td> --}}
                                        <td>
                                            @if (App\Helpers\PermissionHelper::hasPermission('update') || App\Helpers\PermissionHelper::hasPermission('delete'))
                                                ...
                                                @if (App\Helpers\PermissionHelper::hasPermission('delete'))
                                                    <button type="button" data-bs-toggle="modal"
                                                        data-bs-target="#deleteConfirm{{ $item->membership_id }}"
                                    ...
                                    <div class="modal fade" id="deleteConfirm{{ $item->membership_id }}"
                                        ...
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <form action="{{ route('membership.destroy', $item->membership_id) }}"
                                                        method="POST">
                                                        {{ csrf_field() }}
                                                        @method('DELETE')
