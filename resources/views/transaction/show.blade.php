                                        ...
                                        <td>
                                            ...
                                            @if($order->customer->is_member && $order->customer->member)
                                                <span class="badge bg-success">
                                                    {{ $order->customer->member->membership }} ({{ $order->customer->member->discount }}% Discount)
                                                </span>
                                            @else
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div>
                                                    ...
                                                    <small class="text-muted">Category: {{ $detail->product->category->name ?? 'N/A' }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-center">Rp {{ number_format($detail->price_at_purchase, 0, ',', '.') }}</td>
                            ...
                            <div class="card-body">
                                <table class="table table-borderless">
                                    <tr>
                                        ...
                                        <td class="text-end">Rp {{ number_format($order->total_amount + $order->discount, 0, ',', '.') }}</td>
                                    </tr>
                                    @if($order->discount > 0)
                                    <tr>
                            ...
                            <div class="card-body">
                                <table class="table table-borderless">
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            <span class="badge bg-{{ $order->payment->payment_method === 'cash' ? 'success' : 'primary' }}">
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end">{{ $order->payment->payment_date->format('d M Y, H:i') }}</td>
                                    </tr>
                                    @endif
                                    @if($order->payment->payment_method === 'cash')
                                    <tr>
                                        <td><strong>Amount Paid</strong></td>
                                        <td class="text-end">Rp {{ number_format($order->payment->amount + $order->payment->change, 0, ',', '.') }}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Change</strong></td>
                                        <td class="text-end">Rp {{ number_format($order->payment->change, 0, ',', '.') }}</td>
                                    </tr>
                                    @endif
                                    <tr>
                                        <td><strong>Final Amount</strong></td>
                                        <td class="text-end"><strong>Rp {{ number_format($order->payment->amount, 0, ',', '.') }}</strong></td>
                                    </tr>
                                    @if($order->payment->snap_token && $order->payment->payment_method === 'midtrans')
                                    <tr>
                                        <td><strong>Snap Token</strong></td>
                                        <td class="text-end">
                                            <small class="text-muted">{{ substr($order->payment->snap_token, 0, 20) }}...</small>
                                        </td>
                                    </tr>
                                    @endif
                                </table>
                                ...
                                <div class="alert alert-warning mt-3">
                                    ...
                                    <p class="mb-2">This order is waiting for payment confirmation. Customer needs to complete the payment via Midtrans.</p>
                                    <a href="{{ route('transaction.payment', $order->order_id) }}" class="btn btn-sm btn-primary">
                    ...
                    <div class="col-12">
                        <div class="d-flex justify-content-between">
                            ...
                            <div>
                                ...
                                <form action="{{ route('transaction.destroy', $order->order_id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this transaction?')">
                                    @csrf
                                    @method('DELETE')
...
<div id="printable-receipt" class="d-none">
    <div class="receipt-container" style="max-width: 300px; margin: 0 auto; padding: 20px; font-family: Arial, sans-serif;">
        ...
        <table width="100%" style="font-size: 12px;">
            ...
            <tr>
                ...
                <td align="right">Rp {{ number_format($order->payment->amount + $order->payment->change, 0, ',', '.') }}</td>
            </tr>
            <tr>
                <td><strong>Change:</strong></td>
